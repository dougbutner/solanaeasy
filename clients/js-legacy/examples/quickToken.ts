...
const METADATA_URI = process.env.QUICK_METADATA_URI ?? DEFAULT_METADATA_URI;

/** Env var for mint keypair: base58 private key, or path to keypair JSON (vanity mint address). */
const MINT_KEYPAIR_ENV = 'QUICK_MINT_PRIVATE_KEY_BASE58';
const MINT_KEYPAIR_PATH_ENV = 'QUICK_MINT_KEYPAIR';
...
async function createQuickMint(connection: Connection, payer: Keypair) {
...
async function main() {
    ...
    if (mode === 'create') {
        const { mint, treasury } = await createQuickMint(connection, payer);
        console.log('QUICK mint:', mint.toBase58());
