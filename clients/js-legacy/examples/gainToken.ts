...
const DEFAULT_METADATA_URI = 'https://example.com/gain.json';

const RPC_URL = process.env.RPC_URL ?? clusterApiUrl('devnet');
const KEYPAIR_PATH =
    process.env.KEYPAIR ??
    path.join(process.env.HOME ?? process.env.USERPROFILE ?? '.', '.config', 'solana', 'id.json');
const METADATA_URI = process.env.GAINE_METADATA_URI ?? DEFAULT_METADATA_URI;

/** Env var for mint keypair: base58 private key, or path to keypair JSON (vanity mint address). */
const MINT_KEYPAIR_ENV = 'GAINE_MINT_PRIVATE_KEY_BASE58';
const MINT_KEYPAIR_PATH_ENV = 'GAINE_MINT_KEYPAIR';
...
async function createGaineMint(connection: Connection, payer: Keypair) {
...
async function main() {
    ...
    if (mode === 'create') {
        const { mint, treasury } = await createGaineMint(connection, payer);
        console.log('GAINE mint:', mint.toBase58());
