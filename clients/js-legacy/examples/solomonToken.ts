...
const NAME = 'SOLOMON';
const SYMBOL = 'SOLOMON';
const DECIMALS = 6;
const TOTAL_SUPPLY = 1_998_000n * 10n ** BigInt(DECIMALS);
const TRANSFER_FEE_BPS = 200; // 2%
const MAX_FEE = (TOTAL_SUPPLY * 200n) / 1_998_000n; // 2% of total supply in base units
const DEFAULT_METADATA_URI = 'https://example.com/solomon.json';

const RPC_URL = process.env.RPC_URL ?? clusterApiUrl('devnet');
const KEYPAIR_PATH =
    process.env.KEYPAIR ??
    path.join(process.env.HOME ?? process.env.USERPROFILE ?? '.', '.config', 'solana', 'id.json');
const METADATA_URI = process.env.SOLOMON_METADATA_URI ?? DEFAULT_METADATA_URI;

/** Env var for mint keypair: base58 private key, or path to keypair JSON (vanity mint address). */
const MINT_KEYPAIR_ENV = 'SOLOMON_MINT_PRIVATE_KEY_BASE58';
const MINT_KEYPAIR_PATH_ENV = 'SOLOMON_MINT_KEYPAIR';
...
async function createSolomonMint(connection: Connection, payer: Keypair) {
...
async function main() {
    ...
    if (mode === 'create') {
        const { mint, treasury } = await createSolomonMint(connection, payer);
        console.log('SOLOMON mint:', mint.toBase58());
