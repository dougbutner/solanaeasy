...
/** Deployment file name: OFT.json (default) or OFT-{deploymentName}.json for multi-token (e.g. OFT-QUICK.json). */
const getDeploymentFileName = (deploymentName?: string) =>
    deploymentName ? `OFT-${deploymentName}.json` : 'OFT.json'
 ...
 * @param deploymentName {string} Optional token name (QUICK, SOLID, SOLOMON) to save to OFT-{name}.json and avoid overwriting.
 */
export const saveSolanaDeployment = (
    ...
    const fileName = getDeploymentFileName(deploymentName)
    writeFileSync(
        `${outputDir}/${fileName}`,
        JSON.stringify(
            {
    ...
    console.log(`Accounts have been saved to ${outputDir}/${fileName}`)
}

/**
 * Reads the OFT deployment info from disk for the given endpoint ID.
 * @param eid {EndpointId}
 * @param deploymentName {string} Optional token name (QUICK, SOLID, SOLOMON) to load OFT-{name}.json.
 * @returns The contents of the deployment file as a JSON object.
 */
 ...
 * @param deploymentName Optional token name (QUICK, SOLID, SOLOMON) to load OFT-{name}.json.
 */
export const getOftStoreAddress = (eid: EndpointId, deploymentName?: string): string | null => {
    try {
        const { oftStore } = getSolanaDeployment(eid, deploymentName)
        if (!oftStore) {
