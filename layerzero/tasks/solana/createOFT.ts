...
interface CreateOFTTaskArgs {
    ...
    /**
     * Deployment name for multi-token (QUICK, SOLID, SOLOMON). Saves to OFT-{deploymentName}.json.
     */
...
task('lz:oft:solana:create', 'Mints new SPL Token and creates new OFT Store account')
    ...
    .addParam('deploymentName', 'Token deployment name (e.g. QUICK, SOLID, SOLOMON) for OFT-{name}.json', undefined, devtoolsTypes.string, true)
    .setAction(
        async ({
            amount,
            eid,
